...
const addMessage = evt => {
	...
	socket.emit("messageChat-new", messageComplete, id => {
		// callback para obtener el id del mensaje
		console.log(id);
	});
...
getProductsCart = async () => {
	...
	await fetch(
		`https://proyecto-de-backend-production.up.railway.app/carrito/${emailId}`
	)
		.then(res => res.json())
		.then(data => {
...
sendOrder.addEventListener("click", async () => {
	...
	fetch(
		`https://vacuous-temper-production.up.railway.app/ordenes`,
		requestOptions
	)
		.then(res => res.json())
		.then(data => {
